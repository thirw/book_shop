...
class OrderProvider {
  ...
  Future<dynamic> createOrder(Map<String, dynamic> data) async {
    try {
      final response = await api.post(url: ApiPath.ORDER, data: jsonEncode(data));
  ...
  Future<dynamic> getOrderById(String id) async {
    try {
      ...
      final response = await api.getQueryParameter(
          url: ApiPath.ORDER, queryParams: queryParams);
      final  orderResponse = (response!.data as List).map((e) => OrderByID.fromJson(e as Map<String, dynamic>)).toList();
